...
/**
 * Override module used with Modules.override(new DefaultGlueModule()).with(this)
 * to neutralize the consumer side. We only replace SdcRemoteDevicesConnector
 * with a NO-OP so provider graph stays intact and no consumer graph is pulled in.
 */
public final class MedstormProviderOnlyGlueModule extends AbstractModule {
    @Override
    protected void configure() {
        bind(SdcRemoteDevicesConnector.class)
            .toInstance(noopConnector());
    }

    private static SdcRemoteDevicesConnector noopConnector() {
        return (SdcRemoteDevicesConnector) Proxy.newProxyInstance(
